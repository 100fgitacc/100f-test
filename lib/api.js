...
export async function getToken() {
  try {
    const response = await axios.post('https://lunarent.com.ua/test/?rest_route=/simple-jwt-login/v1/auth', {
      email: 'franklins@email.com',
      password: 'franklins@email.com',
    });
...
export async function refreshToken(currentToken) {
  try {
    const response = await axios.post('https://lunarent.com.ua/test/?rest_route=/simple-jwt-login/v1/refresh', {
      token: currentToken,
    });
    cachedToken = response.data.data.jwt;
    return cachedToken;
...
export async function getAllPosts(token) {
  try {
    ...
    const posts = await Promise.all(response.data.map(async (item) => {
      ...
      const domain = 'https://lunarent.com.ua/test/blog_post/';
      elems.link = item.link.replace(domain, '');
      elems.author_name = 'Unnamed Author';
      if (item.author) {
        try {
          const authorResponse = await axios.get(`https://lunarent.com.ua/test/wp-json/wp/v2/users/${item.author}`, {
            headers: {
              Authorization: `Bearer ${token}`
            }
          });
...
//     const response = await axios.get('https://lunarent.com.ua/test/wp-json/wp/v2/pages', {
//       headers: {
//         Authorization: `Bearer ${token}`
//       }
//     });
...
//         }
//       });
//       const domain = 'https://lunarent.com.ua/test/';
//       elems.link = item.link.replace(domain, '');
//       return elems;
//     });
...
//     const response = await axios.get('https://lunarent.com.ua/test/wp-json/wp/v2/popular', {
//       headers: {
//         Authorization: `Bearer ${token}`
//       }
//     });
...
//         }
//       });
//       const domain = 'https://lunarent.com.ua/test/popular/';
//       elems.link = item.link.replace(domain, '');
//       return elems;
//     });
...
export async function getRelatedPosts(token, postId) {
  try {
    const response = await axios.get(`https://lunarent.com.ua/test/wp-json/yarpp/v1/related/${postId}`, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    });
    ...
    const posts = await Promise.all(relatedPostsIds.map(async postId => {
      const response = await axios.get(`https://lunarent.com.ua/test/wp-json/wp/v2/all_posts/${postId}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
...
export async function getAuthorInfo(token, id) {
  try {
    const authorResponse = await axios.get(`https://lunarent.com.ua/test/wp-json/wp/v2/users/${id}`, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    });
...
export async function getPostViews(token, postId) {
  try {
    const response = await axios.get(`https://lunarent.com.ua/test/wp-json/100fapi/v1/get-views/?post_id=${postId}`, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    });
